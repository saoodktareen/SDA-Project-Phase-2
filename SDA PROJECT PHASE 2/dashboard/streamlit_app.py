...
if st.session_state["step"] == "input":
        ...
        with st.form(key="manual_analysis_form"):
            ...
            with col2:
                ...
                decline_years = st.number_input(
                    ...
                    min_value=2,                # changed to 2
                    max_value=20,
                    value=None,                 # initially empty (None)
                    step=1,
                    format="%d",
                    help="Number of consecutive recent years to check for GDP decline per country (minimum 2 years required)"
                )
    ...
    # ── Execute when submitted – with ALL fields required check ──
    if 'submitted' in locals() and submitted:
        ...
        if not is_upload_mode:
            # ── Require EVERY field to have at least one value ──
            all_fields_filled = (
                bool(regions) and                     # at least one region selected
                bool(selected_countries) and           # at least one country selected
                target_year is not None and            # single year filled
                start_year is not None and             # start year filled
                end_year is not None and               # end year filled
                decline_years is not None              # decline years filled
            )

            if not all_fields_filled:
                st.error("All fields are required in manual mode. Please fill at least one value in every field: "
                         "Regions, Countries, Single Target Year, Start Year, End Year, and Years of Decline.")
                st.stop()

            # ── Existing year range validation ──
    ...
    if not results:
        ...
        if selected:
            ...
            if data:
                ...
                if typ == "decline":
                    st.markdown("#### GDP Trend – Selected Countries")
                    for entry in data:
                        ...
                        # Filter chart to show exactly the checked period + small context
                        period_df = chart_df[(chart_df["Year"] >= start_y) & (chart_df["Year"] <= end_y)]
                        if period_df.empty:
                            period_df = chart_df  # fallback to all if no match
                        ...
                        fig.update_layout(
                            ...
                            xaxis_range=[start_y - 1, end_y + 1],  # zoom to period + 1 year margin
                            showlegend=False,
                            height=400
                        )
                        ...
                        st.caption(f"{country}: {status} over checked period ({start_y}–{end_y})")
                        st.markdown("---")
                else:
                    ...
                    NORMAL_COLOR = "#64748B"
                    # ── Modern multi-color palette for pie charts ──
                    PIE_COLORS = [
                        "#FF6B6B", # Coral Red
                        "#4ECDC4", # Turquoise
                        "#45B7D1", # Sky Blue
                    ...
                    if config.get("region"):
                        user_selected.extend(config["region"] if isinstance(config["region"], list) else [config["region"]])
                    if typ in ["top10", "bottom10"]:
                        ...
                        fig = px.pie(
                            ...
                            names="Continent",
                            color="Continent", # color by continent instead of is_selected
                            color_discrete_sequence=PIE_COLORS, # new multi-color palette
                            title=selected["title"],
                            hole=0.3
                        )
                        # Highlight selected continents
                        pull = [0.08 if continent in user_selected else 0 for continent in df["Continent"]]
                        fig.update_traces(
                            pull=pull,
                            marker_line_color=[SELECTED_COLOR if c in user_selected else "white" for c in df["Continent"]],
                            marker_line_width=3
                        )
